	...
	while (!force_quit) {
		for (i = 0; i < nr_queues; i++) {
			nb_rx = rte_eth_rx_burst(port_id,
						i, mbufs, 32);			// 收包
			if (nb_rx) {						// 从所有队列读取数据包并打印目标队列的每个数据包
				for (j = 0; j < nb_rx; j++) {
					...
					*/
					...
					rte_pktmbuf_free(m); // 包打印完成释放占用的mempool
				}
			}
		}
	}
...
{
	struct rte_eth_link link;				// 检测网卡链路信息的结构体
	uint8_t rep_cnt = MAX_REPEAT_TIMES;

	memset(&link, 0, sizeof(link));
	do {
		rte_eth_link_get(port_id, &link);	// 获取链路的状态
		/*
		1. 开启或关闭（ON、OFF）
		2. 链路速度（单位 Mbps）
		3. 通信模式（半双工 or 全双工）
...
init_port(void)
{
	...
	ret = rte_eth_dev_configure(port_id,
				nr_queues, nr_queues, &port_conf);	// 配置网口
	if (ret < 0) {
...
main(int argc, char **argv)
{
	...
	force_quit = false;
	signal(SIGINT, signal_handler);		// 中断型号捕获
	signal(SIGTERM, signal_handler);	// 程序结束(terminate)型号捕获
	...
	// 将目的地ip等于192.168.1.1的数据包发送到队列号1进行过滤
