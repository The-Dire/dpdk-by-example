...
设定一个回调函数,绑定的port以及绑定的rx queue上没收到一个pkt都会触发回调
参数四个:
  1. port id
  2. queue index
  3. 回调函数的函数指针
  4. 传递给回调函数的参数指针
返回值是一个指针,是用于删除回调的api

rte_eth_add_tx_callback()就是为 tx queue 上的发包配置回调函数。参数同上

*/

/*
回调函数的函数指针的格式 rte_rx_callback_fn 详解:
1. rx
typedef uint16_t(* rte_rx_callback_fn)(uint16_t port_id, uint16_t queue, struct rte_mbuf *pkts[], uint16_t nb_pkts, uint16_t max_pkts, void *user_param)
  参数:
    1. port id
    2. queue index
    3. 接收的pkt所存放的 rte_mbuf 的数组
    4. burst中数据包的数量
    5. 可以存储在pkts数组中的最大数据包数
    6. 初始化配置(回调的配置)时,传入的任意用户参数
...
typedef uint16_t(* rte_tx_callback_fn)(uint16_t port_id, uint16_t queue, struct rte_mbuf *pkts[], uint16_t nb_pkts, void *user_param)
  参数:
    ...
    4. burst中数据包的数量
    5. 初始化配置(回调的配置)时,传入的任意用户参数
*/

// rx队列的回调函数,添加(接收时刻的)时间戳
static uint16_t
...
{
	unsigned i;
	uint64_t now = rte_rdtsc(); // 获取当前时间

	for (i = 0; i < nb_pkts; i++)
		pkts[i]->udata64 = now; // mbuf中允许存放8字节用户自定义数据,放在udata64
	return nb_pkts;
}
// tx队列的回调函数,发送时计算时延
static uint16_t
		...
		struct rte_mbuf **pkts, uint16_t nb_pkts, void *_ __rte_unused)
{
	...
	for (i = 0; i < nb_pkts; i++) {
		cycles += now - pkts[i]->udata64; // 记录所有收到的包的时延
		if (hw_timestamping)
...
// 和basicfw是一摸一样的,只是增加了rte_eth_add_tx_callback和rte_eth_add_rx_callback来注册回调
/*
