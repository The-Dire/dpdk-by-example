...
static int port_init(
  ...
  /* rte_eth_dev_configure() 配置网卡
      四个参数
      1. port id
      2. 要给该网卡配置多少个收包队列 这里rx_rings个收包队列
      3. 要给该网卡配置多少个发包队列 也是0个发包队列
      4. 结构体指针类型 rte_eth_conf * 
      为什么这里的0个发包队列，很简单dpdkcap无须发包仅仅需要收包即可
  */
  ...
  */
  ...
  // 如果不止一个队列才需要,这里两个队列的统计会到一个dpdk端口里
  /* Stats bindings (if more than one queue) */
  if(dev_info.max_rx_queues > 1) {
    for (q = 0; q < rx_rings; q++) {
      // 设置指定接收队列到指定每队列统计计数器的映射,用于流量统计
...
int main(int argc, char *argv[]) {
  ...
  struct core_capture_config * cores_config_capture_list; // 抓包核心配置实例
  struct core_write_config   * cores_config_write_list;   // 写包核心配置实例
  ...
  int ret = rte_eal_init(argc, argv); // 初始化dpdk运行环境
  if (ret < 0)
  ...
  arguments = (struct arguments) {
    .statistics = 0,
      ...
      .per_port_c_cores = 1,
      .num_w_cores = 1,
      .no_compression = 0,                  // 是否进行压缩,非压缩情况生成的文件是原始pcap
      .snaplen = PCAP_SNAPLEN_DEFAULT,
      .portmask = 0x1,
      .rotate_seconds = 0,
      .file_size_limit = 0,                 // 文件大小限制
      .log_file=NULL,                       // 写入的文件路径
  };
  ...
  // 生成的文件如果不是压缩的为.pcap是压缩的后缀是.pcap.lzo
  if(!arguments.no_compression)
    strcat(arguments.output_file_template, ".lzo");

  /* Check if at least one port is available 根据不同的dpdk版本使用不同函数 */
#if RTE_VERSION >= RTE_VERSION_NUM(18,11,3,16)
  dev_count=rte_eth_dev_count_avail(); // 获取可用于应用程序的端口数
#else
  ...
  /* Creates the port list */ // 创建所需的dpdk端口
  nb_ports = 0;
  for (i = 0; i < 64; i++) {
  ...
  /* Core index */
  // 这里代码有问题的,直接获取MAIN core即可
  // rte_get_next_lcore(-1, 1, 0);即为获取MAIN core id
  // 而rte_get_next_lcore(-1, 0, 0);即为获取第一个非MAIN core id
  // core_index = rte_lcore_id();
  ...
  for (i=0; i<arguments.num_w_cores; i++) {
    ...
    //Launch writing core
    /*
        rte_eal_remote_launch -- 在另一个 lcore 上启动函数,只能在MAIN lcore上面执行
        第一个参数 要调用的函数。
        第二个参数 待调用函数的参数。
        第三个参数 worker_id(lcore的id) 会在该id的lcore处执行该函数
    */
    // 第一次执行到这里的core_index是在main函数获取的必为MAIN core
    ...
    nb_lcores++;
    /*
        rte_get_next_lcore 获取下一个可用的lcore的id
        第一个参数: 当前 lcore
        第二个参数: 如果为 true，则不返回主 lcore 的 ID。
        第三个参数: 如果为 true，则在达到 RTE_MAX_LCORE 时返回到 0；否则，返回 RTE_MAX_LCORE。
    */
    // 第二个参数非常绕，当其为CALL_MASTER(为1)时可能获取到主MAIN core的id
    // 此时执行rte_eal_remote_launch则主MAIN core也有可能会执行该函数
    // 这里第二个和第三个参数置为false保证rte_eal_remote_launch执行的函数不会再挂在MAIN core
  ...
  for (i = 0; i < nb_ports; i++) {
    port_id = portlist[i];
    // dpdk端口初始化,用于收包,其第一个参数是dpdk(网卡)端口号
    // 第二个参数是收包队列个数，当队列数量大于1时配置网卡RSS
    // 第三个参数是接收队列分配的接收描述符(接收队列大小)
    // 第四个参数是指向内存池mempool的指针,从中分配mbuf去操作队列
    /* Port init */
    ...
    // 抓包模块初始化
    /* Capturing cores */
    for (j=0; j<arguments.per_port_c_cores; j++) {  // 分配多少个核心就要初始化多少个抓包模块
      //Configure capture core
      struct core_capture_config * config =
        &(cores_config_capture_list[i*arguments.per_port_c_cores+j]);
      config->ring = write_ring;                    // 抓包模块的ring赋值
      config->stop_condition = &should_stop;        // 是否停止相应参数
      config->stats =
        &(cores_stats_capture_list[i*arguments.per_port_c_cores+j]);    // 
      config->port = port_id;                       // dpdk端口id
      config->queue = j;                            // 对应的core值方便统计
      //Launch capture core
      // core_index经历过 write_core流程后必然发生了改变,保证了写入模块绑定的核心与抓包模块不同
  ...
  if (arguments.statistics && !should_stop) {
    ...
    start_stats_display(&sd); // 定时输出统计值
    should_stop=true;
  }
  ...
  for(i=0;i<nb_lcores;i++) {
    result = rte_eal_wait_lcore(lcoreid_list[i]); // 等待 lcore 完成其工作
    if (result < 0) {
