...
static uint32_t gLocalIp = MAKE_IPV4_ADDR(192, 168, 0, 119); // 本地的ip,即dpdk绑定网卡的ip
...
static int ng_encode_udp_pkt(uint8_t *msg, unsigned char *data, uint16_t total_len) {
	...
	// 数据放到udp头之下,udp+1为以udp hdr为一单元偏移。就是偏移到udp hdr末尾
...
static struct rte_mbuf *ng_send_arp(struct rte_mempool *mbuf_pool, uint8_t *dst_mac, uint32_t sip, uint32_t dip) {
	...
	const unsigned total_length = sizeof(struct rte_ether_hdr) + sizeof(struct rte_arp_hdr); // 14 + 28,eth头14字节,arp头28字节
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		for (i = 0;i < num_recvd;i ++) {
			...
			// 对udp做处理,这里做的处理是echo回对端
			if (iphdr->next_proto_id == IPPROTO_UDP) {

				struct rte_udp_hdr *udphdr = (struct rte_udp_hdr *)(iphdr + 1);

#if ENABLE_SEND
				// 发送包需要的六元组剩下的dmac,sip,dip,sport,dport获取
