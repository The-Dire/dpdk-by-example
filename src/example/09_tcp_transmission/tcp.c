	...
	for (host = lhost; host != NULL; host = host->next) { // 遍历所有udp socket结点
		...
		if (dstmac == NULL) {
			...
			struct rte_mbuf *arpbuf = ng_send_arp(mbuf_pool, RTE_ARP_OP_REQUEST, gDefaultArpMac, 
				ol->sip, ol->dip);
			// 放入到out buffer中即可.发送在while循环中out buffer把packet出队列发送
			struct inout_ring *ring = ringInstance();
			rte_ring_mp_enqueue_burst(ring->out, (void **)&arpbuf, 1, NULL);
...
struct ng_tcp_stream { // tcb control block
	...
	uint8_t localmac[RTE_ETHER_ADDR_LEN]; // mac地址,发数据的时候装填
	// 缓存ack和syn number
...
// 单例模式,初始化tcp table,tcp table里面保存所有stream信息
...
// 查找tcp stream(连接)
static struct ng_tcp_stream * ng_tcp_stream_search(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport) { // proto就是tcp
	// 获取tcp table
...
static struct ng_tcp_stream * ng_tcp_stream_create(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport) { // proto
	...
	// 这里状态设置为LISTENE状态.在服务器的时候初始状态为LISTEN.客户端则为CLOSE.这里为了简化暂时没有区分统一为服务器端
...
static int ng_tcp_handle_listen(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr) {
	...
	if (tcphdr->tcp_flags & RTE_TCP_SYN_FLAG)  { // tcp头带有SYN FLAG时,要进行listean状态处理
		...
		if (stream->status == NG_TCP_STATUS_LISTEN) {
			...
			fragment->acknum = ntohl(tcphdr->sent_seq) + 1; // 大端序网络字节序要转为小端字节序
...
static int ng_tcp_handle_syn_rcvd(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr) {
	...
	if (tcphdr->tcp_flags & RTE_TCP_ACK_FLAG) { // tcp标志中存在ACK标志
		...
		if (stream->status == NG_TCP_STATUS_SYN_RCVD) {
			...
			stream->status = NG_TCP_STATUS_ESTABLISHED; // 改变tcp状态为ESTABLISHED
		}
...
static int ng_tcp_handle_established(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr, int tcplen) {
	...
	if (tcphdr->tcp_flags & RTE_TCP_SYN_FLAG) { // 出现SYN状态
		//
	} 
	if (tcphdr->tcp_flags & RTE_TCP_PSH_FLAG) { // 出现push状态

		// 1. recv buffer -> 接收到后传递给应用程序的包
		...
		int payloadlen = tcplen - hdrlen * 4; // tcp payload总长度
		if (payloadlen > 0) {
			// 拷贝到tcp包中即rfragment中
			...
			memset(rfragment->data, 0, payloadlen+1); // +1是为了保存\0
			// 队友
		...
		// 放入到stream流的recv buffer中(用户态应用程序可以从中取出包)
		rte_ring_mp_enqueue(stream->rcvbuf, rfragment); 
		// 2. ack pkt (回给客户端的ack包)
	...
	if (tcphdr->tcp_flags & RTE_TCP_ACK_FLAG) { // 出现ack状态

	}
	if (tcphdr->tcp_flags & RTE_TCP_FIN_FLAG) { // 出现FIN状态

		stream->status = NG_TCP_STATUS_CLOSE_WAIT; // 设置stream的状态为CLOSE_WAIT
...
static int ng_tcp_process(struct rte_mbuf *tcpmbuf) {
	...
	tcphdr->cksum = 0; // 先置0才能计算
	uint16_t cksum = rte_ipv4_udptcp_cksum(iphdr, tcphdr);
#if 0 // 调式用.这里最初我把tcpcksum做了大端转小端然后出错了,发现不需要转换.这里应该是rte_ipv4_udptcp_cksum做了处理
	if (cksum != tcpcksum) {
	...
	if (stream == NULL) { // 没有找到stream,创建tcp stream(stream是三次握手后才有的)
...
static int ng_tcp_out(struct rte_mempool *mbuf_pool) {
	...
	for (stream = table->tcb_set;stream != NULL;stream = stream->next) {
		...
		// 如果目的ip存在与arp table中
		uint8_t *dstmac = ng_get_dst_macaddr(stream->sip); // dmac是客户端的源ip
		if (dstmac == NULL) {
			// 目的mac为空,发送arp request
			struct rte_mbuf *arpbuf = ng_send_arp(mbuf_pool, RTE_ARP_OP_REQUEST, gDefaultArpMac, 
				stream->dip, stream->sip);
			// 发送arp包不直接发送,直接放入到out buffer中即可
			struct inout_ring *ring = ringInstance();
			rte_ring_mp_enqueue_burst(ring->out, (void **)&arpbuf, 1, NULL);
